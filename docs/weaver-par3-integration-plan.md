...
Planning record saved on 2026-09-08. This is future Weaver work, not a claim
that the integration has been implemented. The immediate task remains fixing
rarpar PR #67. Resume this plan only when Weaver implementation is requested.
...
- Consume published crates, not Git or external path patches. A published
  engine containing the advanced APIs and late review fixes is a prerequisite.
  Publication, deployment, and production operations are outside this plan.
