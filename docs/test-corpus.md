...
The PAR3 recipe also generates the `advanced/` cases through
`bench/rarpar-bench/internal/testcorpus/par3_advanced.go`: low-rate FFT in both
fields, uneven interleaved cohorts, and PAR inside ZIP, ZIP64, and 7z. Original
archives are paired with the reference's inserted outputs. ZIP inputs use Go's
archive writer; a deterministic 7z Copy archive requires no additional toolchain.
The initial container runs used Python ZIP and native 7-Zip inputs, recorded in
the fixture README; regeneration uses the checked-in input writers. All PAR3
bytes are unmodified output of the pinned reference executable.
