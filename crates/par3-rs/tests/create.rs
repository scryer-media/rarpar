...
/// PR #73 round 4, finding D. `Readme` and `README` are one name on a
/// case-insensitive filesystem — macOS and Windows by default — where the
/// second file written takes the first one's place. A set naming both could
/// never be repaired back onto the filesystem it came from, so creation refuses
/// the pair instead of writing a set that cannot be installed.
#[test]
fn two_input_names_that_differ_only_by_letter_case_are_refused() {
    let tree = TempTree::new("case-folded-inputs");
    // On a case-insensitive filesystem these two writes are one file, which is
    // exactly the collision under test; on a case-sensitive one they are two.
    tree.write("Readme", &filler(1, 3000));
    tree.write("README", &filler(2, 3000));

    let files = [PathBuf::from("Readme"), PathBuf::from("README")];
    let options = CreateOptions::default()
        .with_block_size(1024)
        .with_recovery(RecoveryAmount::Blocks(1));
    let error = create(
    ...
    .expect_err("a set naming one file twice, in two cases, is not writable anywhere");
    let reason = error.to_string();
    assert!(
        reason.contains("letter case"),
        "refused for the wrong reason: {reason}"
    );
    assert!(
        !tree.path().join("case.par3").exists(),
        "the set was written before the names were checked"
    );

    // The same two names in different directories are two different paths, and
    // stay allowed: they collide with nothing.
    tree.write("one/Readme", &filler(3, 3000));
    tree.write("two/README", &filler(4, 3000));
    let spread = [PathBuf::from("one/Readme"), PathBuf::from("two/README")];
    create(
        &InputSpec::new(tree.path(), &spread),
        &tree.path().join("spread"),
        &options,
    )
    .expect("two names that only share a spelling in different directories are fine");
}
