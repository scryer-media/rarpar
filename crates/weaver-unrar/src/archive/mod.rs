...
use crate::signature;
use crate::types::{
    ArchiveFormat, ArchiveMetadata, CompressionMethod, FileHash, IndexedMemberInfo, MemberInfo,
    RecoveryRecordInfo, RecoveryRecordKind, TopologyMemberInfo, UnixOwnerInfo, VolumeSpan,
};
...
impl RarArchive {
    ...
    /// List member metadata with stable indices suitable for extraction calls.
    pub fn indexed_member_infos(&self) -> Vec<IndexedMemberInfo> {
        self.members
            .iter()
            .enumerate()
            .map(|(index, entry)| {
                let info = self.make_member_info(entry);
                let mut missing_volumes = (info.volumes.first_volume..=info.volumes.last_volume)
                    .filter(|volume| !self.volume_set.is_present(*volume))
                    .collect::<Vec<_>>();
                if entry.file_header.split_after {
                    let next_volume = info.volumes.last_volume + 1;
                    if !self.volume_set.is_present(next_volume) {
                        missing_volumes.push(next_volume);
                    }
                }
                let extractable = !entry.file_header.split_before && missing_volumes.is_empty();
