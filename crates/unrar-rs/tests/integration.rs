...
#[test]
#[ignore = "requires rar4/rar4_hp_long_password.rar: RARPAR_FIXTURES_LONG_PASSWORD=1 cargo run -p xtask -- test-corpus generate --only edge_cases"]
fn test_rar4_hp_long_password_fixture_gate() {
    let path = fixture("rar4", "rar4_hp_long_password.rar");
    assert!(
        path.exists(),
        "missing real RAR4 long-password fixture; it is not corpus content, so generate it on \
         demand with RARPAR_FIXTURES_LONG_PASSWORD=1 cargo run -p xtask -- test-corpus generate \
         --only edge_cases"
    );
...
// =============================================================================
// Edge-case fixtures — written by the `edge_cases` recipe
// =============================================================================
...
/// (`bench/rarpar-bench/internal/testcorpus/ppmd_solid.go`).
#[test]
