...
impl RarArchive {
    ...
    pub(super) fn open_boxed_inner(
        ...
        let volume_number = match main.and_then(|m| m.volume_number) {
    ...
    pub(super) fn rar4_to_file_header(
        ...
        FileHeader {
            ...
            host_os: match fh.host_os {
                ...
                // Only Darwin/macOS, Linux/Unix, and Windows behavior is
                // mapped. Other RAR4 host ids are legacy archive origins, not
                // supported extraction targets, so keep them intentionally
                // unmapped.
...
mod tests {
    ...
    /// ClusterFuzzLite `rar_headers` OOM, GitHub Actions run 31924958641.
    ///
    /// sha256 `485ced425e9253be3a6baa6f3c796bfbc098662c3b727570f5c03bd5d8741e3b`;
    /// the same 130 bytes are seeded at
    /// `fuzz/corpus/rar_headers/main-header-huge-volume-number`. The main header
    /// sets VOLUME | VOLUME_NUMBER and declares volume 508_427_613_235_168_135,
    /// which `VolumeSet::add_volume` used to turn into a dense `Vec<VolumeState>`
    /// — 462_412 TiB of `push`, i.e. abort before a member byte is read.
    const FUZZ_OOM_HUGE_VOLUME_NUMBER: &[u8] = &[
        ,
    ];

    #[test]
    fn open_rejects_main_header_volume_number_above_cap_instead_of_allocating() {
        match RarArchive::open(std::io::Cursor::new(FUZZ_OOM_HUGE_VOLUME_NUMBER.to_vec())) {
            Err(RarError::ResourceLimit { .. }) => {}
            Err(err) => panic!("expected a resource-limit rejection, got {err}"),
            Ok(_) => panic!("expected a resource-limit rejection, got success"),
        }
    }
}
