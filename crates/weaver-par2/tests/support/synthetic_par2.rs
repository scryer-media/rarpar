//! Shared synthetic-PAR2 scaffolding for repair integration tests: a
//! deterministic PRNG, an in-memory PAR2 set builder, and packet framing.
//! Included via `#[path]` by each test binary that needs it (repair_stress,
//! wgpu_stream_repair), so items may be unused in any single binary.
#![allow(dead_code)]
...
pub fn make_full_packet(packet_type: &[u8; 16], body: &[u8], recovery_set_id: [u8; 16]) -> Vec<u8> {
