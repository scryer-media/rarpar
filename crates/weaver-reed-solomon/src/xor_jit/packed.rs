//! Packed reconstruction dispatch for the AVX2 XOR-JIT tier.
//!
//! Ordered single-input bodies deduplicated by coefficient. (The AVX-512
//! multi-source packed tier was removed after c5 hardware measurement; git
...
#[cfg(test)]
use super::deps;
use super::{JitWidth, avx2_emitter, memory::JitCode};
...
///
/// Reports the generated bytes and aligned immutable arena retained for one
/// active input group.
...
pub struct PackedScratch {
    // Kept for API shape; the pointer-marshalling belonged to the removed
    // AVX-512 multi-source arm.
    #[allow(dead_code)]
...
///
/// Generated bodies share one active-group RX arena, deduplicated by
/// coefficient across rows.
...
impl PackedJitBatch {
    ...
    pub fn memory_upper_bound(
        ...
        let generated_bytes = code_index_bytes.checked_add(mem::size_of::<GeneratedBodies>())?;
...
impl PackedJitCode {
    ...
    pub unsafe fn try_run_with_scratch(
        &self,
        // Retained for API shape: the pointer-marshalling scratch belonged to
        // the removed AVX-512 multi-source arm; the AVX2 arm needs none.
        _scratch: &mut PackedScratch,
        run: PackedRun,
    ) -> Result<(), PackedExecutionError> {
        self.try_run_with_scratch_inner(_scratch, run)
    }
    ...
    pub unsafe fn run_with_scratch(
        ...
        self.try_run_with_scratch(_scratch, run)
    }
...
mod tests {
    ...
    fn packed_memory_bound_covers_build_and_pointer_storage() {
        {
            let estimate = PackedJitBatch::memory_upper_bound(JitWidth::Avx2, 1, 12).unwrap();
    ...
    /// The create path builds one multi-row batch per input batch through the
    /// workspace (every band row in one sealed arena, the per-row rebuild it
    /// replaced cost c5 60% of create in construction), recycles it, and the
    /// same workspace immediately serves the next batch.
    #[test]
    fn workspace_multi_row_batch_builds_and_recycles() {
        let mut workspace = PackedJitWorkspace::default();
        let rows = [vec![0x1AFFu16; 12], vec![0x0101u16; 12], vec![7u16; 12]];
        for _input_batch in 0..3 {
            let row_refs: Vec<&[u16]> = rows.iter().map(Vec::as_slice).collect();
            let batch = workspace
                .build(JitWidth::Avx2, &row_refs, usize::MAX)
                .unwrap();
